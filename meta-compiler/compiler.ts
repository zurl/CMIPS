...
import {IMyTokenizer, IMyParser} from "./compiler.def";
import testcode from './bnf.test';
...
const tokenHashMap = {};
let tokenHashNow = 0;
function getTokenHash(str: string){
    if( tokenHashMap.hasOwnProperty(str))
        return tokenHashMap[str];
    else{
        if(/^[a-z]$/.test(str))tokenHashMap[str] = str;
